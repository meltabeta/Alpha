// recommend section of video play
...
function renderCards(cards, page) {
  ...
  for (let i = start; i < end; i++) {
    ...
    anchor.href = card.recommendList; // Set the href to the card's recommendList path
...
function filterCards(query) {
  // Filter based on type "Anime"
  let filteredByType = cardsData.cards.filter(card => card.type === 'Anime');
...
fetch('../../home-playlist.json')
  .then(response => response.json())
  .then(data => {
    cardsData = data; // Store the data for later use

    // Filter the cards to include only "Anime" type and sort them
    filteredCards = sortCardsByLastUpdated(cardsData.cards.filter(card => card.type === 'Anime'));
    renderCards(filteredCards, currentPage);
